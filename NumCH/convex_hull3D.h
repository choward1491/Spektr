...
public:
    ...
    =========================================*/
    Convex_Hull3D( T * x_arr, T * y_arr, T * z_arr, int number_pts );
    ...
    function to output a vector parametrized
     by type T triangles
    =========================================*/
    void getData( vector< Triangle<T> > & out_struct ); // returns final data
...
private:
    ...
    =========================================*/
    void getConvexHullHelper(Triangle<T> * tri, MaxHeap<T,int> & heap );
...
template< typename T >
Convex_Hull3D<T>::Convex_Hull3D(){
    x = 0;
    y = 0;
    z = 0;
    num_pts = 0;
    data = HashTable< int, Triangle<T>, AVL_Tree<int, Triangle<T>> >(&ch3d_hf);
}

template< typename T >
Convex_Hull3D<T>::Convex_Hull3D( T * x_arr, T * y_arr, T * z_arr, int number_pts ){
    x = x_arr;
    y = y_arr;
    z = z_arr;
    num_pts = number_pts;
    data = HashTable< int, Triangle<T>, AVL_Tree<int, Triangle<T>> >(&ch3d_hf, number_pts/2);
...
void Convex_Hull3D<T>::generateConvexHull(){
    ...
    if ( x != 0 ) {
        ...
        MaxHeap<T, int> points1;
        MaxHeap<T, int> points2;
        MaxHeap<T, int> points3;
        MaxHeap<T, int> points4;
        ...
        tri2.makeNormal(x, y, z, num_pts);
        tri2.id = 1;
        
        assign<int>(tri3.vertices, i3, i1, i4);
        tri3.makeNormal(x, y, z, num_pts);
        tri3.id = 2;
        
        assign<int>(tri4.vertices, i3, i2, i1);
        tri4.makeNormal(x, y, z, num_pts);
        tri4.id = 3;
        ...
        for (int i = 0; i < num_pts; i++) {
            if( i != i1 && i != i2 && i != i3 && i != i4 ){
                ...
                if ( dist1 > zero ) {
                    points1.push( dist1, i );
                }
                
                if (dist2 > zero ){
                    points2.push( dist2, i );
                }
                
                if (dist3 > zero ){
                    points3.push( dist3, i );
                }
                
                if (dist4 > zero ){
                    points4.push( dist4, i );
                }
        ...
        data.push(tri1.id, tri1);
        data.push(tri2.id, tri2);
        data.push(tri3.id, tri3);
        data.push(tri4.id, tri4);
...
template< typename T >
void Convex_Hull3D<T>::getData( vector< Triangle<T> > & out_struct ){
...
template< typename T >
void Convex_Hull3D<T>::getConvexHullHelper(Triangle<T> * tri, MaxHeap<T,int> & heap ){
    
    if ( heap.getHeapCount() > 0 ) {
        
        // Get number of items in table
        int cnt = data.getNumberOfItemsInTable();
        
        // Find the max point index
        int ind  = heap.pop();
        int hcnt = heap.getHeapCount();
        
        // Get copy of input triangle
        bool check = false;
        Triangle<T> old_tri = data.remove(tri->id, check);
        
        // Get copy of neighbors of input triangle
        Triangle<T> n1 = data.find(tri->neighbors[0], check);
        Triangle<T> n2 = data.find(tri->neighbors[1], check);
        Triangle<T> n3 = data.find(tri->neighbors[2], check);
        ...
        for (int i = 0; i < neighbor_inds.getCount(); i++ ) {
            int tmp = neighbor_inds.pop();
        ...
        if ( hcnt > 0 ){
            MaxHeap<T,int> points1( hcnt );
            MaxHeap<T,int> points2( hcnt );
            MaxHeap<T,int> points3( hcnt );
            ...
            for (int i = 0; i < hcnt; i++) {
                ...
                if ( dist1 > zero ) {
                    points1.push( dist1, ind );
                }
                
                if (dist2 > zero ){
                    points2.push( dist2, ind );
                }
                
                if (dist3 > zero ){
                    points3.push( dist3, ind );
                }
            }// end for i hcnt loop
